...
def main():
    ...
    test_processed_path = os.path.join(RESULTS_DIR, 'X_test_processed.csv')
    ...
    for idx, row in results_df.iterrows():
        ...
        logger.info("-"*70)
        # ====================================================================
        # END OF FEATURE IMPORTANCE BLOCK
        # ====================================================================
        ...
        # Save parquet
        parquet_path = os.path.join(RESULTS_DIR, f'submission_rank{rank}_valrmse_{row["val_rmse"]:.4f}.parquet')
        submission_df.to_parquet(parquet_path, index=False, engine='fastparquet')
        logger.info(f"[+] Parquet saved: {parquet_path}")
        
        # Save CSV
        csv_path = os.path.join(RESULTS_DIR, f'submission_rank{rank}_valrmse_{row["val_rmse"]:.4f}.csv')
    ...
    logger.info("="*70)
    logger.info(f"\n✓ Best Model: Rank #1 with Validation RMSE = {summary_df.iloc[0]['val_rmse']:.4f} kg")
