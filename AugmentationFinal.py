...
# Paths
DATA_DIR = Path('data')
RESULTS_DIR = Path('Results/checkpoint_Augmentation_Submission')
PLOTS_DIR = RESULTS_DIR / 'plots'
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
PLOTS_DIR.mkdir(parents=True, exist_ok=True)

# ============================================================================
# AIRCRAFT SUPPORTED BY OPENAP (36 types)
# ============================================================================
...
def estimate_with_openap_correct_mass(aircraft_type, full_traj, interval_traj, interval_start, phase):
    """OpenAP only for supported aircraft"""
    try:
        # KEY: Check if aircraft is supported FIRST
        if aircraft_type not in AIRCRAFT_DATA:
            return None, f"Unsupported aircraft: {aircraft_type}"
...
def run(force=False):
    """Run OpenAP augmentation for the final dataset."""
    output_file = RESULTS_DIR / 'augmented_openap_submission_ALL_FLIGHTS.csv'
    ...
    print(" Processing ALL 24288 Flight IDs from fuel_final_submission.parquet")
    ...
    print(" SUBMISSION MODE - Using flights_final dataset")
    print("="*80)

    print("\n[Loading data...]")
    fuel_df_raw = pd.read_parquet(DATA_DIR / 'fuel_final_submission.parquet')
    flightlist_df = pd.read_parquet(DATA_DIR / 'flightlist_final.parquet')
    ...
    parquet_files = list((DATA_DIR / 'flights_final/flights_final').glob('*.parquet'))

    flight_data = {}
    print(f"\n[Loading trajectories from flights_final...]")
