...
def add_features_trajectories(df):
    ...
    # Estimate tasx and tasy assuming heading is the same as track
    tasx = df.tas * np.sin(df.track)
    tasy = df.tas * np.cos(df.track)
    
    # groundspeed = airspeed + wind
    # u east v north
    u_component_of_wind = gsx - tasx
    v_component_of_wind = gsy - tasy
    
    # Add missing columns with NaNs
    df['temperature'] = np.nan
    df['specific_humidity'] = np.nan
    ...
    return df.assign(
        ...
        wind=np.hypot(u_component_of_wind, v_component_of_wind)
    )
