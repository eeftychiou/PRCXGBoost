"""
This script handles the model training stage of the ML pipeline using Gradient Boosting Regressor.
...
log_file = os.path.join('logs', f'gbr_fuel_openap_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
logging.basicConfig(
    level=logging.INFO,
...
def train(params_path=None, selected_features_path=None):
    ...
    """
    print("--- Starting Gradient Boosting Model Training Stage ---")
    ...
    model_params = {'n_estimators': 100, 'learning_rate': 0.1, 'max_depth': 5, 'random_state': 42, 'loss': 'huber'}
    ...
    print("Training final Gradient Boosting Regressor model...")
    ...
    if X_val is not None and y_val is not None:
        ...
        y_pred_log = gbr.predict(X_val_processed)
    ...
    model_dir = os.path.join(config.MODELS_DIR, f"gbr_model{model_suffix}_{run_id}")
    os.makedirs(model_dir, exist_ok=True)

    joblib.dump(gbr, os.path.join(model_dir, "model.joblib"))
    ...
    print("--- Gradient Boosting Model Training Stage Complete ---")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Gradient Boosting Model Training")
